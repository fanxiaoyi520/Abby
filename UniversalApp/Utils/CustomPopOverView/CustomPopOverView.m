...
- (void)setLayerFrame:(CGRect)frame
{
    ...
    if (_finalPosition == FinalPositionLeft) {
        CGFloat maxY = CGRectGetHeight(frame);
    ...
    if (_style.shadowColor) {
        ...
        self.popLayer.shadowOpacity = 0.09;
...
- (void)updateSubViewFrames
{
    ...
    switch (_contentPosition) {
        ...
        case CPContentPositionAlwaysLeft:
        {
            self.containerView.finalPosition = FinalPositionLeft;
            containerViewFrame.origin.y = CGRectGetMinY(newFrame)-self.content.height+CGRectGetHeight(newFrame);
            contentFrame.origin.y = self.style.roundMargin;
        }
            break;
    ...
    switch (_alignStyle) {
        ...
        {
            CGRect frame = self.containerView.frame;
            frame.origin.x = CGRectGetMinX(newFrame) - (fabs(frame.size.width - newFrame.size.width))-self.style.showSpace-_showFrom.width;
            self.containerView.frame = frame;
