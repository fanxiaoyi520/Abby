...
@interface HDMessageViewController ()<HDMessageCellDelegate,HDChatManagerDelegate,TransmitDeleteTrackMsgDelegate, UIGestureRecognizerDelegate,HRobotUnsolveItemViewDelegate,UIViewControllerTransitioningDelegate,ABOxygenRewardDelegate>
{
...
@property (nonatomic ,strong) UILabel *o2NumLab;
@end
...
- (void)viewDidLoad {
    ...
    self.chatToolbar = [[HDChatToolbar alloc] initWithFrame:CGRectMake(0, self.view.height - chatbarHeight - CONTACTS_SAFE_BOTTOM, self.view.width, chatbarHeight)];
...
- (void)setLeftBarBtnItem {
    ...
    [backButton setImage:[UIImage imageNamed:@"icon_t_arrow"] forState:UIControlStateNormal];
...
- (void)setChatToolbar:(HDChatToolbar *)chatToolbar
{
    ...
    tableFrame.size.height = self.view.frame.size.height - _chatToolbar.frame.size.height-CONTACTS_SAFE_BOTTOM;
...
- (void)chatToolbarDidChangeFrameToHeight:(CGFloat)toHeight
{
    [UIView animateWithDuration:0.3 animations:^{
        ...
        rect.origin.y = 0;
        rect.size.height = self.view.frame.size.height - toHeight - CONTACTS_SAFE_BOTTOM;
...
- (void)add_moreViewTakePicAction:(HDChatBarMoreView *)moreView {
    // Hide the keyboard
    [self.chatToolbar endEditing:YES];
...
- (void)add_oxygenRewardAction:(id)sender {
    ABOxygenRewardViewController *vc = ABOxygenRewardViewController.new;
...
// MARK: ABOxygenRewardDelegate
- (void)oxygenReward_sureBtnAction:(NSString *)o2NumStr {
    self.o2NumLab.text = o2NumStr;
    [self.o2ImgView insertSubview:self.o2NumLab atIndex:0];
    
    UIGraphicsImageRendererFormat *format = [[UIGraphicsImageRendererFormat alloc] init];
       format.prefersExtendedRange = YES;
    UIGraphicsImageRenderer *renderer = [[UIGraphicsImageRenderer alloc] initWithSize:self.o2ImgView.size format:format];
    __weak typeof(self) weakSelf = self;
    UIImage *tmpImage = [renderer imageWithActions:^(UIGraphicsImageRendererContext * _Nonnull rendererContext) {
        return [weakSelf.o2ImgView.layer renderInContext:rendererContext.CGContext];
    }];
...
- (UIImageView *)o2ImgView {
    if (!_o2ImgView) {
        ...
        _o2ImgView.image = ImageName(@"pic_chat_O2");
        _o2ImgView.frame = CGRectMake(0, 0, _o2ImgView.image.size.width, _o2ImgView.image.size.height);
    }
    return _o2ImgView;
}
...
- (UILabel *)o2NumLab {
    if (!_o2NumLab) {
        _o2NumLab = UILabel.new;
        _o2NumLab.font = FONT_BOLD(13);
        _o2NumLab.textColor = [UIColor colorWithHexString:@"0x006241"];
        _o2NumLab.textAlignment = NSTextAlignmentCenter;
        _o2NumLab.numberOfLines = 0;
        _o2NumLab.frame = CGRectMake(0, (self.o2ImgView.height-21)/2, self.o2ImgView.width, 21);
    }
    return _o2NumLab;
}
@end
