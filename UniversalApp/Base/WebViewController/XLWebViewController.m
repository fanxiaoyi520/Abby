...
@interface XLWebViewController () <WKNavigationDelegate,WKScriptMessageHandler>
...
-(instancetype)initWithUrl:(NSString *)url {
    self = [self initWithUrl:url withNavTitle:nil];
    return self;
}

-(instancetype)initWithUrl:(NSString *)url withNavTitle:(NSString *)navTitle {
    self = [super init];
    if (self) {
        self.ntitle = navTitle;
        self.url = [NSString stringWithFormat:@"%@%@",WEB_URL_main,url];
...
- (void)setNavRightStr:(NSString *)navRightStr {
    [self addNavigationItemWithTitles:@[navRightStr] isLeft:NO target:self action:@selector(rightBtnClick:) tags:@[@(2001)]];
}

- (void)viewDidLoad {
    [super viewDidLoad];
    [self initWKWebView];
    self.navTitle = self.ntitle;
    //适配iOS11
...
-(void)initWKWebView
{
    ...
    _jsHandler = [[XLJSHandler alloc] initWithViewController:self configuration:configuration withDelegate:self];
