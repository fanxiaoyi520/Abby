...
//UIKIT_EXTERN CGFloat KFTApplicationItemsCCell_Height;
@interface ABTrendCell ()
...
- (void)showMoreBtnAction:(UIButton *)sender {
    ...
    [self theContentLineShowsMoreLogic];
    if ([self.delegate respondsToSelector:@selector(trend_updeteCellHeight:)]) {
        [self.delegate trend_updeteCellHeight:sender];
    }
}
...
- (void)bottomFuncAreaAction:(UIButton *)sender {
    switch (sender.tag) {
        case 100:
            if ([self.delegate respondsToSelector:@selector(trend_LikeAction:)]) {
                [self.delegate trend_LikeAction:sender];
            }
            break;
        case 101:
            if ([self.delegate respondsToSelector:@selector(trend_giftAction:)]) {
                [self.delegate trend_giftAction:sender];
            }
            break;
        case 102:
            if ([self.delegate respondsToSelector:@selector(trend_downloadAction:)]) {
                [self.delegate trend_downloadAction:sender];
            }
            break;
        case 103:
            if ([self.delegate respondsToSelector:@selector(trend_moreAction:)]) {
                [self.delegate trend_moreAction:sender];
            }
            break;
    }
}
...
- (void)deviceDataPopAction:(UITapGestureRecognizer *)tap {
    if ([self.delegate respondsToSelector:@selector(trend_deviceDataPopAction:)]) {
        [self.delegate trend_deviceDataPopAction:tap];
    }
}
