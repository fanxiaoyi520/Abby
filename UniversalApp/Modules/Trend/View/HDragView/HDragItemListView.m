...
- (CGFloat)itemListH
{
    ...
    return CGRectGetMaxY([(HDragItemListView *)self.items.lastObject frame]) + _itemMargin;
}
...
- (void)setup
{
    ...
    _itemSize = CGSizeMake(width, ratioH(123));
...
- (void)closeDeleteItem:(UIButton *)sender {
    HDragItem *tagButton = (HDragItem *)sender.superview;
...
- (void)addItem:(HDragItem *)item
{
    ...
    if (!item.isAdd) [item addSubview:closeBtn];
    [closeBtn setImage:ImageName(@"icon_p_xx") forState:UIControlStateNormal];
    closeBtn.frame = CGRectMake(_itemSize.width-22-8, 8, 22, 22);
    [closeBtn addTarget:self action:@selector(closeDeleteItem:) forControlEvents:UIControlEventTouchUpInside];
...
- (HDragItem *)buttonCenterInButtons:(HDragItem *)curItem
{
    for (HDragItem *button in self.items) {
        ...
        //坐标转换
        CGRect rect = [self.superview convertRect:self.frame toView:[UIApplication sharedApplication].keyWindow];
        CGRect frame = CGRectMake(button.originX + rect.origin.x, button.originY + rect.origin.y, button.width, button.height);
