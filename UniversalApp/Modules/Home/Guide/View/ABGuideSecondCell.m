...
@implementation ABGuideSecondCell
- (void)updateSelStatus:(BOOL)isSel {
    self.selBtnImgView.image = isSel ? ImageName(@"Controls Small_Checkboxes_Ok_1") : ImageName(@"Controls Small_Checkboxes_Ok");
}

- (void)setModel:(ABGuideSecondModel *)model {
    _model = model;
...
- (void)selBtnAction:(UIButton *)sender {
    ...
    if ([self.delegate respondsToSelector:@selector(guideSecondCell_selFuncAction:)]) {
        [self.delegate guideSecondCell_selFuncAction:sender];
    }
}
...
- (UIButton *)selBtn {
    if (!_selBtn) {
        ...
        _selBtn.backgroundColor = [UIColor colorWithHexString:@"0xF7F7F7"];
        _selBtn.selected = NO;
    }
    return _selBtn;
}
